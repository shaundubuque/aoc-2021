...
fn most_common_bits(report: &Vec<String>) -> String {
    ...
    counters
        .iter()
        .map(|bit_sum| if *bit_sum >= 0 { "1" } else { "0" })
        .collect()
}
...
fn matcher_bit_for_index(
    ...
    for line in report {
        counter += if line.chars().nth(index).unwrap() == '0' {
            -1
        } else {
            1
        };
...
fn least_common_bits(most_common_bits: &String) -> String {
    most_common_bits
        .chars()
        .map(|bit_char| if bit_char == '0' { '1' } else { '0' })
        .collect()
}
...
fn find_match(entries: &Vec<String>, match_fn: fn(&Vec<String>, usize) -> u8) -> String {
    ...
    for i in 0..entries.first().unwrap().len() {
        ...
        working_set = working_set
            .iter()
            .filter(|entry| entry.as_bytes()[i] == match_bit as u8)
            .cloned()
            .collect();
